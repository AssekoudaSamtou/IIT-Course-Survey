<template>
    <v-app>
        ...
        <v-card>
            ...
            <v-data-table id="data-table-x" v-model="selected" :headers="headers" :items="terms" :search="search" @click:row="onTableRowSelected"/>
        ...
        <div class="text-xs-center">
            <v-dialog v-model="dialog" width="500">
                ...
                <v-card>
                    <v-card-title class="headline grey lighten-2" primary-title>Terms form</v-card-title>
    ...
    export default {
        ...
        methods: {
            onTableRowSelected: function (data) {
                this.termForm.code = data.code;
                this.termForm.name = data.name;
            ...
            save: function () {
                let data = {
                    ...
                    code: this.termForm.code,
                    name: this.termForm.name,
                };

                if (this.crud_mode === "c") {
                    this.$http.post(`${this.$store.state.backendHostName}/services/save/term`, data, {
                        headers: {
                    ...
                    for (let i = 0; i < this.terms.length; i++) {
                        if(this.terms[i].id === this.selectedRow) {
                            console.log('id: ', i);
            ...
            remove: function () {
                for (let i = 0; i < this.terms.length; i++) {
                    if(this.terms[i].id === this.selectedRow) {
                        ...
                        this.$http.get(`${this.$store.state.backendHostName}/services/delete/term/${this.selectedRow}`, {
                            params:{},
                            headers: {
                                'Authorization': `${this.$store.state.userInfo.tokenType} ${this.$store.state.userInfo.accessToken}`,
                                'Content-Type': 'application/json',
